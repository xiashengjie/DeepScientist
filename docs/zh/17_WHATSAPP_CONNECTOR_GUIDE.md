...
当你希望通过 WhatsApp 继续推进 Uniresearch quest 时，请阅读这份指南。
...
8. 回到 Uniresearch，确认目标会话已经被自动发现。
...
Uniresearch 绑定的是这个规范化后的 conversation id，而不是临时浏览器状态或短期 session 状态。
...
- 运行时状态会镜像写入 Uniresearch 的 connector 日志目录
- 入站消息会从本地 session inbox 中被消费
- 出站消息会被写入本地 session outbox

这也符合 Uniresearch 的本地优先设计。
