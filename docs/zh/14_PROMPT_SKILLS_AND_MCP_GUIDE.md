...
这篇文档解释的是：Uniresearch 每一轮到底是怎么被驱动起来的。
...
Uniresearch 不是靠一份静态大 prompt 工作的。
...
- Uniresearch 仍然会优先读取 `quest_root/.codex/prompts/` 下存在的 prompt 片段。
- 但这棵树现在是“自动维护的受管副本”，不再默认视为永久手工 fork。
- 每次真实 runner turn 开始前，Uniresearch 都会把当前 active quest-local prompt 树与仓库 `src/prompts/` 做比较；只要仓库源变了，或者 quest 本地副本漂移了，就会刷新 active 副本。
...
所以 Uniresearch 虽然是同一套 runtime，但在这些场景里可以表现不同：
...
Uniresearch 不是随机往 prompt 里塞 memory 的。
...
- 每次真实运行前，Uniresearch 都会把 active quest-local copy 修回当前仓库 prompt 真相。
...
Uniresearch 会把它解析成“该正式版本号下最新的一份 prompt 备份”。如果你想精确回放某一次具体备份，而不是该版本下最新的一份，也仍然可以直接传备份目录名。
...
Uniresearch 的 daemon 不应该变成一个巨大的硬编码科研调度器。
...
这也是 Uniresearch 更像一个持续科研工作坊，而不是无状态聊天的原因。
...
- prompt 组装顺序或 runtime context 选择错了 -> 改 `src/Uniresearch/prompts/builder.py`
- 内建工具面本身不对 -> 改 `src/Uniresearch/mcp/server.py`
