...
1. 明确 Uniresearch 的开源协议与责任边界
2. 明确你在实际部署和使用时需要自己承担的主要风险

这不是法律意见。如果你要把 Uniresearch 用在公司环境、生产环境、真实业务系统或对外服务中，请让你自己的法务、安全或运维团队再审一次。

## 1. 开源协议与责任边界

Uniresearch 采用 Apache 2.0 开源协议发布。
...
- Uniresearch 按 Apache 2.0 协议发布
- Uniresearch 按“现状”提供，不提供任何明示或暗示担保
- 因使用、部署、修改、二次开发、共享或暴露 Uniresearch 而产生的任何直接或间接后果，项目作者与维护者概不负责
- 你需要自行负责运行环境、权限设置、对外暴露、第三方账号绑定、结果审查、数据处理和合规问题

换句话说，Uniresearch 是一个高能力自动化系统，不是托管服务，也不是安全审计后的托底产品。你必须自己控制边界。
...
Uniresearch 可以驱动模型执行命令、修改文件、安装依赖、运行脚本、读写项目目录。
...
Uniresearch 是基于模型的自动化系统，不保证结论真实。
...
如果你使用 `0.0.0.0`、公网 IP、反向代理、隧道服务或域名把 Uniresearch 暴露到外部，你实际上是在暴露一个具备自动化执行能力的系统。
...
Uniresearch 可以与 QQ、微信、Lingzhu 等外部平台结合使用。
...
Uniresearch 可能持续运行、多轮调用模型、下载依赖、执行实验、生成文件。
...
如果你打算真正长期使用 Uniresearch，至少做到下面这些：

### 3.1 优先使用 Docker 或其他隔离环境

强烈建议优先在 Docker 容器、虚拟机或同等级隔离环境中运行 Uniresearch，而不是直接在宿主机高权限环境里裸跑。
...
1. Uniresearch 采用 Apache 2.0 协议发布。
2. 项目作者与维护者不对你使用 Uniresearch 造成的任何后果负责。
