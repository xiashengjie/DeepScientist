...
Uniresearch 不会为 MiniMax、GLM、火山方舟、阿里百炼单独实现一套 provider 适配器。

如果你要使用阿里百炼上的 Qwen，需要特别注意：Uniresearch 只支持 **Coding Plan** 这条路径，不支持普通百炼 / DashScope 平台的 Qwen API。
...
如果你希望 Uniresearch 之后默认就走这个 profile，可以写进 `runners.yaml`：
...
- Uniresearch 现在会在 `.ds/codex-home` 下启动一个隔离的运行时 home，但会先继承你配置的 `~/.codex` 里的 auth、config、skills、agents 与 prompts
- 如果当前生效 provider 使用的是 `wire_api = "chat"`，Uniresearch 现在会在启动探测时自动检查所选 Codex binary 是否正好是 `0.57.0`

## Provider 一览

| Provider | 官方文档 | 是否需要 Codex 登录 | Uniresearch 应该怎么用 |
|---|---|---|---|
...
- 本地实测 Uniresearch 可稳定跑通的组合包括 `MiniMax-M2.7` + `m27` + Codex CLI `0.57.0`，以及 `MiniMax-M2.5` + `m25` + Codex CLI `0.57.0`
- 当前最新版 `@openai/codex` 和 MiniMax 官方文档并不能稳定直接对齐

如果你现在要走最稳的 Uniresearch + MiniMax 路径，建议直接使用 Codex CLI `0.57.0`。

### 需要准备什么

- 已安装 Codex CLI `0.57.0`
- 已创建 MiniMax `Coding Plan Key`
- 在启动 Codex 和 Uniresearch 的 shell 中可见的 `MINIMAX_API_KEY`
...
现在如果 Uniresearch 在启动时检测到 MiniMax profile，但当前 `codex` 版本不是 `0.57.0`，在交互式终端里会主动提示是否自动安装 `@openai/codex@0.57.0`。
...
如果你走的是 Uniresearch 路径，那么当所选 provider 明确写了 `requires_openai_auth = false` 时，Uniresearch 现在会在 startup probe 和真实 runner 执行时自动剥离 `OPENAI_API_KEY` 和 `OPENAI_BASE_URL`。
...
- 可复现的 Uniresearch 路径是：`MiniMax-M2.7` 配 `m27`，以及 `MiniMax-M2.5` 配 `m25`
- 对 `m25` 这条路径，请使用 `MiniMax-M2.5`，不要写成 `codex-MiniMax-M2.5`

因此，下面给的是当前 Uniresearch 推荐配置：
...
如果你想走同样的 Uniresearch 路径但使用 `m25`，保持上面的 provider 配置不变，再把 profile 写成：
...
Uniresearch 现在对它的支持方式是：

- 如果你使用的是这类 profile-only MiniMax 配置，再配合 Codex CLI `0.57.0`，Uniresearch 会在自己的 probe / 运行时临时 `config.toml` 副本里，把所选 profile 的 `model_provider` 和 `model` 自动提升到顶层
- 对 provider-backed 的 MiniMax profile，Uniresearch 会强制使用 `model: inherit`，避免再被硬编码的 OpenAI 模型覆盖
- 当 `requires_openai_auth = false` 时，Uniresearch 会自动移除冲突的 `OPENAI_API_KEY` 和 `OPENAI_BASE_URL`
- 对 MiniMax 这类 `Codex CLI 0.57.0 + wire_api = \"chat\"` 的会话，Uniresearch 现在会额外注入一个兼容约束，明确要求 Codex 把 MCP 工具调用串行化，一次只发一个 tool call，而不是把多个 tool call 打包进同一轮回复
- 这意味着即使终端里原样执行 `codex --profile m27` 还会失败，Uniresearch 也可以先兼容跑起来
...
Uniresearch 现在会为 MiniMax 的 `0.57.0` 路径额外做两层兼容：

- 当检测到旧版 Codex CLI 不支持 `xhigh` 时，自动把 `xhigh` 降级成 `high`
- 当检测到 MiniMax 使用 profile-only 的 `model_provider` / `model` 配置形态时，在临时 Uniresearch Codex home 里自动补齐顶层字段
...
如果 provider-backed profile 在 Uniresearch 里还是失败：
...
4. Uniresearch 里优先保持 `model: inherit`
